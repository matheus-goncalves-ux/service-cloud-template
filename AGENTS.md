...
| prototype-setup | `.agent/skills/prototype-setup/SKILL.md` | First-time setup of the starter kit after the user opened the workspace: Copilot auto-approve, Claude model, optional Figma MCP, smoke test, optional branding (logo + color). Use when the user just opened the repo and asks "how do I start", "set up from scratch", "configurar pela primeira vez", "acabei de clonar". |
